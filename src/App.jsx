...
export default function App() {
  const [tab,        setTab]        = useState('dashboard')   // 'dashboard' | 'data'
  const [running,    setRunning]    = useState(false)
  const [validated,  setValidated]  = useState(false)
  const [roiData,    setRoiData]    = useState(INITIAL_ROI)
  const [bets,       setBets]       = useState(INITIAL_BETS)
  const [matchData,  setMatchData]  = useState(null)          // uploaded matches
  ...
  const handleRun = ({ text, stakeMode, threshold, parsed }) => {
    ...
    setTimeout(() => {
      ...
      // Rebuild ROI curve from backtest bets
      let bank = 1000
      const newRoi = []
      for (let i = 0; i < 52; i++) {
      ...
      setKpis({
        ...
        avgEV:    result.bets.length > 0
          ? result.bets.reduce((s, b) => s + (b.ev ?? 0), 0) / result.bets.length
          : 0,
        kelly: result.bets.length > 0
          ? result.bets.reduce((s, b) => s + (b.kelly ?? 0), 0) / result.bets.length
          : 0,
      })
    ...
    <div style={{ background: C.bg0, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 9, color: C.muted }}>v0.1.0</span>

          {/* Nav tabs */}
          <div style={{ marginLeft: 24, display: 'flex', gap: 0 }}>
            {[['dashboard', '▦  DASHBOARD'], ['data', '⬆  DATA']].map(([k, l]) => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          {matchData && (
            <Tag color={C.green}>{matchData.length} MATCHES LOADED</Tag>
          )}
          <span style={{
            fontSize:  9,
            color:     C.green,
            ...
            <Panel title="Format Guide" icon="ℹ" accent={C.blue}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ fontSize: 11, color: C.textD, lineHeight: 1.9 }}>
                  ...
                  <span style={{ color: C.amberL }}>.json</span> file containing
                  your match statistics. The engine will automatically detect
                  available fields and calculate whichever metrics it can.
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    ...
                    <div key={label} style={{
                      ...
                      <div style={{ fontSize: 9, color: C.textD, lineHeight: 1.7 }}>
                        {fields}
                      </div>
                    </div>
                  ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.45fr', gap: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <Panel
                  ...
                  accent={C.amber}
                  headerRight={
                    <Tag color={validated ? C.green : C.muted}>
                ...
                <Panel title="Execution Log" icon="◉" accent={C.blue}>
                  ...
                  >
                    ...
                    {running && (
                      <span style={{ color: C.amber, animation: 'blink 0.8s infinite' }}>▋</span>
                    )}
                  </div>
                </Panel>
              </div>
